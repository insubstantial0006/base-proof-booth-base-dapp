...
const source = await readFile(resolve(process.cwd(), "contracts/BaseProofBooth.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "BaseProofBooth.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["BaseProofBooth.sol"]?.BaseProofBooth;
...
console.log(`Deploying BaseProofBooth from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_PROOF_BOOTH_CONTRACT_ADDRESS to this contract address in Vercel.");
